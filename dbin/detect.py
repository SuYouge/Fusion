...
def detect(save_txt=False, save_img=False, stream_img=False):
    ...
    task = []
    # task = MyThread(mSerial.read_data)
    # task.start()
    n = 0
    for path, img, im0, vid_cap in dataset:
        ###
        tsk = MyThread(mSerial.read_data)
        task.append(tsk)
        task[n].start()
        # print(task[n].is_alive())
        # p = task[n].get_result()
        n += 1
        # mSerial.get_dist()
        # task[n].join()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='cfg/yolov3-tiny-2cls.cfg', help='cfg file path')
    parser.add_argument('--data', type=str, default='data/ball.data', help='coco.data file path')
    parser.add_argument('--weights', type=str, default='weights/tiny-thresh06.pt', help='path to weights file')
